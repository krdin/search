...
class ControllerProductSearch extends Controller {
		public function index() {
			...
			// Шаг 1: Загрузка модели Sphinx
			$this->load->model('catalog/sphinxsearch_unique');  
			
			$this->load->model('tool/image'); 

			// Шаг 2: Получение поискового запроса от пользователя
			$search = isset($this->request->get['search']) ? $this->request->get['search'] : '';

			// Шаг 3: Получение результатов поиска от Sphinx
			if (!empty($search)) {
        $results = $this->model_catalog_sphinxsearch_unique->search($search);
    } else {
        $results = array();
    }
		...
		if (isset($this->request->get['search']) || isset($this->request->get['tag'])) {
			...
			$url = '';
				...
				// Получение результатов поиска Sphinx
				$this->load->model('catalog/sphinxsearch_unique');
				$search_query = $this->request->get['search'];
				$sphinx_results = $this->model_catalog_sphinxsearch_unique->search($search_query);

				$data['sphinx_results'] = $sphinx_results; 
				file_put_contents(DIR_LOGS . 'sphinx_results_debug.log', print_r($sphinx_results, true));
