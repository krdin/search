...
require_once(__DIR__ . '/sphinx.php');

if (!class_exists('ModelCatalogSphinxsearch')) {
    class ModelCatalogSphinxsearchUnique extends ModelCatalogSphinx {
...
public function connectToSphinx() {

    // Устанавливаем соединение с Sphinx через MySQL протокол (порт 9306)
    $mysqli = new mysqli('localhost', '', '', '', 0, '/home/kuhnisol/.system/sphinx/sphinx-mysql.socket');
    
    // Проверяем соединение
    if ($mysqli->connect_error) {
        file_put_contents(DIR_LOGS . 'sphinx_search_log.txt', "Connection error: " . $mysqli->connect_error . "\n", FILE_APPEND);
        die("Ошибка подключения: " . $mysqli->connect_error);
    } 
    file_put_contents(DIR_LOGS . 'sphinx_search_log.txt', "Successfully connected to Sphinx.\n", FILE_APPEND);
    
    return $mysqli;
}
							public function search($searchQuery, $index = NULL, $wildcard = false) {
    // Подключение к серверу Sphinx
    $mysqli = $this->connectToSphinx();

    // Определяем индекс для поиска на основе текущего языка
    $index = ($this->language->get('code') == 'uk') ? 'src_products_uk' : 'src_products';

    // Отправляем поисковый запрос через SphinxQL
   $result = $mysqli->query("SELECT * FROM src_products, src_products_uk WHERE MATCH('$searchQuery')");
    ...
    file_put_contents(DIR_LOGS . 'sphinx_search_log.txt', "Sphinx search method called with query: " . $searchQuery . "\\nSearch results count: " . count($data) . "\\n", FILE_APPEND);

    return $data;
}
}
class ModelCatalogSphinxsearch extends Model {
    protected $_fields = array(
        'main' => array(
            'columns' => array(
                'id'           => 'int',
